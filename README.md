...
- Builder aplicado em (domain/space.py) agora se tornando (space_builder.py). O ganho mais concreto aparece no dynamic_space_registration_service.py, que antigamente recebia um dicionário e construia o Space manualmente. Com o Builder, cada etapa do cadastro dinâmico (que pode ter campos preenchidos em telas diferentes) vai adicionando atributos de forma encadeada, e o .build() só dispara quando tudo está pronto — validando os campos obrigatórios em um único ponto antes da criação.
